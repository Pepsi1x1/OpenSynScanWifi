...
using SR = OpenSynScanWifi.Properties.Resources;
...
namespace System.Net.Sockets
{
    ...
    {
	    private static class TcpValidationHelpers
	    {
	    ...
	    private static class IPEndPointStatics
	    {
	    ...
	    private static class RangeValidationHelpers
	    {
	    ...
	    private static class ExceptionCheck
	    {
        ...
        public async ValueTask<UdpReceiveResult> ReceiveAsync(CancellationToken token)
        {
	        ThrowIfDisposed();

	        await this._clientSocket.ReceiveAsync(
		        _buffer.AsMemory(),
		        SocketFlags.None,
		        token).ConfigureAwait(false);

            return new UdpReceiveResult(_buffer.AsSpan(0, this.Available).ToArray(), (IPEndPoint)this._clientSocket.RemoteEndPoint);
        }
